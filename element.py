...
class Element(object):
    ...
    def __uidump(self):
        ...
        if int(self.utils.get_sdk_version()) >= 19:
            self.utils.get_cmd_result('adb shell uiautomator dump --compressed /data/local/tmp/uidump.xml')
        else:
            self.utils.get_cmd_result('adb shell uiautomator dump /data/local/tmp/uidump.xml')
        self.utils.get_cmd_result('adb pull data/local/tmp/uidump.xml %s' % self.tempFile)
        self.utils.get_cmd_result('adb shell rm /data/local/tmp/uidump.xml')
